# Garden Detective: Weather or Climate?

An accessible, simplified branching scenario for **Master Gardener volunteers**. Learners examine real-world garden evidence, decide whether it supports a weather event, a climate pattern, or a need for more evidence, and receive choice-specific coaching.
...
Do not upload only `index.html`. It depends on `styles.css` and `script.js`.

---

## Test locally

For a quick visual check, double-click `index.html`.

Some institutional browser settings restrict local JavaScript. If the page looks unstyled or does not advance, test through GitHub Pages instead.
...
Scenario content is stored near the top of `script.js` in the `scenarios` array. Preserve each scenario's identifiers and feedback structure when editing.
